...
NDD takes Given/When/Then from BDD, along with the idea that specs should describe behavior in a shared language and be executable.

Where it diverges: BDD scenarios are flat lists by feature. NDD organizes them into time-based narrative structure (narratives > scenes > moments). It also unifies Given/When/Then for domain logic with describe/it/should for interface contracts, which BDD and component testing traditionally kept separate.
...
NDD takes Ubiquitous Language, Bounded Contexts, and the insistence on deep domain understanding before code.

Where it diverges: DDD focuses on domain models and strategic patterns. NDD focuses on user journeys and the narrative structure connecting domain concepts through time. They're complementary, and you'll often want both.
...
What NDD adds is the combination: a single model that pulls together Disney/Airbnb's storyboard-based experience design, BDD's executable specs, EventStorming's collaborative discovery, event sourcing's data integrity, DDD's domain focus, Specification by Example's concrete examples, and Story Mapping's journey-based organization. None of these ideas are new on their own. Wiring them into one coherent model is.
